...
function distributeGifts(packOfGifts: string[], reindeers: string[]): number {
    let weightOfGifts: number = 0;
    let weightOfReindeers: number = 0;
    
    packOfGifts.forEach((a: string) => {
        weightOfGifts += a.split('').length;
    });

    reindeers.forEach((a: string) => {
        weightOfReindeers += a.split('').length * 2;
    });

    let division: number = Math.round(weightOfGifts / weightOfReindeers);

    return (division * weightOfGifts <= weightOfReindeers) ? division : 0;
}
...
console.log(distributeGifts(['balon', 'camisa', 'correa', 'celular'], ["dasher", "dancer"])); // 1
console.log(distributeGifts(['balon', 'camisa', 'correa', 'celular', 'peluche', 'almanaque'], ["dasher"])); // 3
